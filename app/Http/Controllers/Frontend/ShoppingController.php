...
public function cart_store(Request $request)
{
    ...
    // ✅ SAFETY: Use variant price if selected, otherwise fallback to product price
    $finalPrice = $request->pro_price && $request->pro_price > 0
        ? $request->pro_price
        : $product->new_price;

    // ✅ BLOCK INVALID CART INSERT (VERY IMPORTANT)
    // if (!$request->pro_price) {
    //     Toastr::error('Please select a product size to see the price', 'Error!');
    //     return redirect()->back();
    // }
    ...
    Cart::instance('shopping')->add([
        ...
        'name'  => $product->name,
        'qty'   => $request->qty,
        'price' => $finalPrice, // ✅ DYNAMIC PRICE APPLIED HERE
        'options' => [
            'slug'           => $product->slug,
