...
<tbody>
    <?php $__currentLoopData = Cart::instance('shopping')->content(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <tr data-price="<?php echo e($value->options->old_price); ?>">
            <td>
            ...
            <td class="text-left">
                ...
                <?php if($value->options->product_size): ?>
                    <p>
                        Size: <?php echo e($value->options->product_size); ?>
                ...
                <?php if($value->options->product_color): ?>
                    <p>
                        Color: <?php echo e($value->options->product_color); ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Update price when quantity + or - clicked
    document.querySelectorAll('.vcart-qty .plus, .vcart-qty .minus').forEach(function(btn){
        btn.addEventListener('click', function() {
            let row = this.closest('tr');
            let basePrice = parseFloat(row.dataset.price);
            ...
            row.querySelector('.cart_price strong').innerText = (basePrice * qty).toFixed(2);
        });
    });

    // Update price when size or color changes
    document.querySelectorAll('.product-size, .product-color').forEach(function(select){
        select.addEventListener('change', function() {
            let row = this.closest('tr');
            let qty = parseInt(row.querySelector('input[type="text"]').value);

            // Get selected option price
            let selectedPrice = parseFloat(this.selectedOptions[0].dataset.price);

            // Update row's base price
            row.dataset.price = selectedPrice;

            // Update total price for that row
            row.querySelector('.cart_price strong').innerText = (selectedPrice * qty).toFixed(2);
        });
    });
                        ...
                        <div class="card-footer text-danger">
                            <?php echo str_replace('href="/shop"', 'href="' . route('shop') . '"', $generalsetting->checkout_note); ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.product-size, .product-color').forEach(function(el) {
...
<?php echo $__env->make('frontEnd.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH F:\laragon\www\ecommerce1.nextstagesoftware.com_20260507_141104\resources\views/frontEnd/layouts/customer/checkout.blade.php ENDPATH**/ ?>