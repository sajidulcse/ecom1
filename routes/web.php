...
// Admin OTP-based password reset routes -- added 2026-04-17
use App\Http\Controllers\Auth\AdminPasswordResetController;
Route::get('admin/password/forgot',       [AdminPasswordResetController::class, 'showForgotForm'])->name('password.request');
Route::post('admin/password/send-otp',    [AdminPasswordResetController::class, 'sendOtp'])->name('password.send.otp');
Route::get('admin/password/verify-otp',   [AdminPasswordResetController::class, 'showVerifyForm'])->name('password.otp.verify.form');
Route::post('admin/password/verify-otp',  [AdminPasswordResetController::class, 'verifyOtp'])->name('password.otp.verify');
Route::get('admin/password/new-password', [AdminPasswordResetController::class, 'showNewPasswordForm'])->name('password.new.form');
Route::post('admin/password/new-password',[AdminPasswordResetController::class, 'updatePassword'])->name('password.new.update');
...
Route::group(['namespace'=>'Admin','middleware' => ['auth','lock','check_refer'],'prefix'=>'admin'], function() {
    ...
    // Dashboard widget config/export routes -- added 2026-05-02
    Route::post('dashboard/widget/orders/config',   [DashboardController::class, 'orderConfig'])->name('dashboard.orders.config');
    Route::get('dashboard/widget/orders/reset',     [DashboardController::class, 'orderReset'])->name('dashboard.orders.reset');
    Route::get('dashboard/widget/orders/export/{format}', [DashboardController::class, 'orderExport'])->name('dashboard.orders.export');
    Route::post('dashboard/widget/customers/config',[DashboardController::class, 'customerConfig'])->name('dashboard.customers.config');
    Route::get('dashboard/widget/customers/reset',  [DashboardController::class, 'customerReset'])->name('dashboard.customers.reset');
    Route::get('dashboard/widget/customers/export/{format}', [DashboardController::class, 'customerExport'])->name('dashboard.customers.export');
    ...
    // Admin profile -- added 2026-04-15
    Route::get('profile', [DashboardController::class, 'profile'])->name('admin.profile');
    Route::post('profile/update', [DashboardController::class, 'profile_update'])->name('admin.profile.update');
    // Clear all caches via optimize:clear -- added 2026-04-15
    Route::get('optimize-clear', function() {
        Artisan::call('optimize:clear');
        return back()->with('success', 'Caches Cleared');
    })->name('admin.optimize.clear');
    ...
    Route::post('courierapi/pathao/refresh-token', [ApiIntegrationController::class,'pathaoRefreshToken'])->name('pathao.refresh_token');
    ...
    // SMTP settings -- added 2026-04-15
    Route::get('settings/smtp', [SmtpSettingController::class,'index'])->name('admin.smtp.index');
    Route::post('settings/smtp/update', [SmtpSettingController::class,'update'])->name('admin.smtp.update');
    // FraudBD settings -- added 2026-04-15
    Route::get('settings/fraud', [FraudSettingController::class,'index'])->name('admin.fraud.setting');
    Route::post('settings/fraud/update', [FraudSettingController::class,'update'])->name('admin.fraud.setting.update');
    // AI chat settings -- added 2026-04-15
    Route::get('settings/ai-chat', [AiChatSettingController::class,'index'])->name('admin.ai_chat.index');
    Route::post('settings/ai-chat/update', [AiChatSettingController::class,'update'])->name('admin.ai_chat.update');
    ...
    // Fraud check -- added 2026-04-15
    Route::get('fraud-check', [OrderController::class, 'fraudCheck'])->name('admin.fraud.check');
    ...
    Route::get('contact-messages/{id}', [ContactMessageController::class,'show'])->name('admin.contact_messages.show');
    Route::delete('contact-messages/{id}', [ContactMessageController::class,'destroy'])->name('admin.contact_messages.destroy');
